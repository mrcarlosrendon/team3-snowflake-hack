...
st.set_page_config(
    page_title="Cold Weather Payment Simulator",
    page_icon="👋",
)
...
st.markdown('<h1 class="heading">COLD WEATHER PAYMENT SIMULATOR</h2><BR>', unsafe_allow_html=True)
...
with st.sidebar:
    st.markdown('<h1 class="tabheading">SELECT OPTIONS</h2><BR>', unsafe_allow_html=True)
    with st.form('Drivers'):
        S.shareddata = 'ORGDATACLOUD$INTERNAL$DATASETS_TO_SUPPORT_THE_COLD_WEATHER_PAYMENT_APP'
...
if submitted:
    ...
    ######## weather data
    summary_data = session.table(f'{S.shareddata}.COLD_WEATHER_PAYMENTS."Hourly Forecast"')
    ...
    weather_station = session.table(f'{S.shareddata}.COLD_WEATHER_PAYMENTS.PCSECTORMAPPING')\
...
if submitted:
    ...
    entitled_payments_sav.write.save_as_table(f'DATA.{st.experimental_user.user_name}_COLD_WEATHER_PAYMENT_SCENARIO',mode="append")
    ...
    st.markdown(f'<p class="blue">YOUR SCENARIO HAS BEEN SAVED AS VERSION NUMBER {S.scenario}</p>', unsafe_allow_html=True)
    ...
    S.geos = session.table(f'{S.shareddata}.COLD_WEATHER_PAYMENTS."Postal Out Code Geometries"')
